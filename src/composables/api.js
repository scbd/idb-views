    ...
    try 
        {
          ...
          const q = { 
                      ...
                      $and: [
                        {'meta.createdOn': { $gte: { $date: startYearDate.toISO() } }},
                        {'meta.createdOn': { $lt:  { $date: endYearDate.toISO() } }}
                      ],
                      ...query
                    };
          const params = { q: JSON.stringify({ ...q}),  f: JSON.stringify({ ...f}) };
          ...
          return data;
            ...
            if(e.message==='Network Error')
                addError({ name: 'Network Error', message:`There appears to be a network connection error between your computer and the end point`, apiUrl});
        
            if(e.response?.status == 404)
                addError({ status: e.response.status, name: e.response.statusText, message:`The document could not be located using identifier`});
        
            consola.error('@scbd/idb-views.composables.api.listDocuments', e);
    }
}
...
export const getDocument = async (identifier = getIdentifierFromQuery()) => {
  try {
    ...
    consola.error('@scbd/idb-views.composables.api.getDocument', e);
  }
}
...
export const postStatus = async (status, identifier = getIdentifierFromQuery()) => {
  try {
    const { apiUrl} = await getOptions();
    const   headers = await getAuth();

    if(!identifier) throw new Error('@scbd/idb-views.composables.api.postStatus: no document identifier specified');
    
    const { status:s, data } = await axios.post(`${apiUrl}/${encodeURIComponent(identifier)}/status/${status}`, {}, { headers } );
    
    if(s == 200) addFeedback(defaultEditMsg);
    ...
    consola.error('@scbd/idb-views.composables.api.postStatus', e);
  }
}
...
export const postDocument = async (document) => {
  try {
    ...
    consola.error('@scbd/idb-views.composables.api.postDocument', e);
  }
}
...
export const putDocument = async (document, identifier = getIdentifierFromQuery()) => {
  try {
    ...
    if(!identifier) throw new Error('@scbd/idb-views.composables.api.putDocument: no document identifier specified');
    ...
    consola.error('@scbd/idb-views.composables.api.putDocument', e);
  }
}
...
function cleanNils (obj) {
  ...
  if(!(isAnArray || isAnObject)) throw new Error('@scbd/idb-views.composables.api.clean- param passed to clean is not an object or array');
